// <auto-generated />
using LuukMuschCustomModelManager.Databases;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace LuukMuschCustomModelManager.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20250208105840_AddBlockModelPath")]
    partial class AddBlockModelPath
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            modelBuilder.Entity("CustomModelData_ParentItem", b =>
                {
                    b.Property<int>("CustomModelDataID")
                        .HasColumnType("int");

                    b.Property<int>("ParentItemID")
                        .HasColumnType("int");

                    b.HasKey("CustomModelDataID", "ParentItemID");

                    b.HasIndex("ParentItemID");

                    b.ToTable("CustomModelData_ParentItem", (string)null);
                });

            modelBuilder.Entity("LuukMuschCustomModelManager.Model.BlockType", b =>
                {
                    b.Property<int>("BlockTypeID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.HasKey("BlockTypeID");

                    b.ToTable("BlockTypes");
                });

            modelBuilder.Entity("LuukMuschCustomModelManager.Model.CustomModelData", b =>
                {
                    b.Property<int>("CustomModelDataID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<int>("CustomModelNumber")
                        .HasColumnType("int");

                    b.Property<string>("ModelPath")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<bool>("Status")
                        .HasColumnType("tinyint(1)");

                    b.HasKey("CustomModelDataID");

                    b.ToTable("CustomModelDataItems");
                });

            modelBuilder.Entity("LuukMuschCustomModelManager.Model.CustomModel_BlockType", b =>
                {
                    b.Property<int>("CustomModelDataID")
                        .HasColumnType("int");

                    b.Property<int>("BlockTypeID")
                        .HasColumnType("int");

                    b.HasKey("CustomModelDataID", "BlockTypeID");

                    b.HasIndex("BlockTypeID");

                    b.ToTable("CustomModel_BlockType");
                });

            modelBuilder.Entity("LuukMuschCustomModelManager.Model.CustomModel_ShaderArmor", b =>
                {
                    b.Property<int>("CustomModelDataID")
                        .HasColumnType("int");

                    b.Property<int>("ShaderArmorColorInfoID")
                        .HasColumnType("int");

                    b.HasKey("CustomModelDataID", "ShaderArmorColorInfoID");

                    b.HasIndex("ShaderArmorColorInfoID");

                    b.ToTable("CustomModel_ShaderArmor");
                });

            modelBuilder.Entity("LuukMuschCustomModelManager.Model.CustomVariation", b =>
                {
                    b.Property<int>("CustomVariationID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("BlockData")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("BlockModelPath")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<int>("BlockTypeID")
                        .HasColumnType("int");

                    b.Property<int>("CustomModelDataID")
                        .HasColumnType("int");

                    b.Property<int>("Variation")
                        .HasColumnType("int");

                    b.HasKey("CustomVariationID");

                    b.HasIndex("BlockTypeID");

                    b.HasIndex("CustomModelDataID");

                    b.ToTable("CustomVariations");
                });

            modelBuilder.Entity("LuukMuschCustomModelManager.Model.ParentItem", b =>
                {
                    b.Property<int>("ParentItemID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.HasKey("ParentItemID");

                    b.ToTable("ParentItems");
                });

            modelBuilder.Entity("LuukMuschCustomModelManager.Model.ShaderArmorColorInfo", b =>
                {
                    b.Property<int>("ShaderArmorColorInfoID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<int>("Color")
                        .HasColumnType("int");

                    b.Property<string>("HEX")
                        .IsRequired()
                        .HasMaxLength(7)
                        .HasColumnType("varchar(7)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("RGB")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("varchar(11)");

                    b.HasKey("ShaderArmorColorInfoID");

                    b.ToTable("ShaderArmorColorInfos");
                });

            modelBuilder.Entity("CustomModelData_ParentItem", b =>
                {
                    b.HasOne("LuukMuschCustomModelManager.Model.CustomModelData", null)
                        .WithMany()
                        .HasForeignKey("CustomModelDataID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LuukMuschCustomModelManager.Model.ParentItem", null)
                        .WithMany()
                        .HasForeignKey("ParentItemID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("LuukMuschCustomModelManager.Model.CustomModel_BlockType", b =>
                {
                    b.HasOne("LuukMuschCustomModelManager.Model.BlockType", "BlockType")
                        .WithMany("CustomModelDataItems")
                        .HasForeignKey("BlockTypeID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LuukMuschCustomModelManager.Model.CustomModelData", "CustomModelData")
                        .WithMany("BlockTypes")
                        .HasForeignKey("CustomModelDataID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("BlockType");

                    b.Navigation("CustomModelData");
                });

            modelBuilder.Entity("LuukMuschCustomModelManager.Model.CustomModel_ShaderArmor", b =>
                {
                    b.HasOne("LuukMuschCustomModelManager.Model.CustomModelData", "CustomModelData")
                        .WithMany("ShaderArmors")
                        .HasForeignKey("CustomModelDataID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LuukMuschCustomModelManager.Model.ShaderArmorColorInfo", "ShaderArmorColorInfo")
                        .WithMany("CustomModelDataItems")
                        .HasForeignKey("ShaderArmorColorInfoID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CustomModelData");

                    b.Navigation("ShaderArmorColorInfo");
                });

            modelBuilder.Entity("LuukMuschCustomModelManager.Model.CustomVariation", b =>
                {
                    b.HasOne("LuukMuschCustomModelManager.Model.BlockType", "BlockType")
                        .WithMany("CustomVariations")
                        .HasForeignKey("BlockTypeID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LuukMuschCustomModelManager.Model.CustomModelData", "CustomModelData")
                        .WithMany("CustomVariations")
                        .HasForeignKey("CustomModelDataID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("BlockType");

                    b.Navigation("CustomModelData");
                });

            modelBuilder.Entity("LuukMuschCustomModelManager.Model.BlockType", b =>
                {
                    b.Navigation("CustomModelDataItems");

                    b.Navigation("CustomVariations");
                });

            modelBuilder.Entity("LuukMuschCustomModelManager.Model.CustomModelData", b =>
                {
                    b.Navigation("BlockTypes");

                    b.Navigation("CustomVariations");

                    b.Navigation("ShaderArmors");
                });

            modelBuilder.Entity("LuukMuschCustomModelManager.Model.ShaderArmorColorInfo", b =>
                {
                    b.Navigation("CustomModelDataItems");
                });
#pragma warning restore 612, 618
        }
    }
}
